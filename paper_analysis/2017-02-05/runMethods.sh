...
then
	...
	echo "source ${bashrc}" > ${s}
	echo "${mypython} ${CODE}/genomedisco/__main__.py --m1 ${f1} --m2 ${f2} --m1name ${firstItem} --m2name ${secondItem} --node_file ${disco_nodes} --outdir ${outdir}/disco/${pair}/ --outpref chr${chromosome} --m_subsample NA --approximation 40000 --norm sqrtvc --method RandomWalks --tmin 3 --tmax 7 --concise_analysis" >> ${s}
	chmod 755 ${s}
	qsub -l h_vmem=20G -l hostname=scg3* -o ${s}.o -e ${s}.e ${s}
	echo "========"
    done < ${metadata}
