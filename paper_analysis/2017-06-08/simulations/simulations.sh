...
nodefile=/ifs/scratch/oursu/encode_highres/results/res40000/data/nodes/nodes.chr21.gz
...
tadmeansize=1000000 #median of real data on chr21
...
then
	...
	do
		...
		do
                    ...
                    chromo="simulated"
                    echo "${dataname_short}.${ab}delim${EDGE_DIR}/${dataname}.gzdelimNA" | sed 's/delim/\t/g' >> ${edgenoise_metadata}
		done
            done
	done
    done
	...
	do
		...
		do
                    dataname=${dataname_basic}.${ab}
                    chromo="simulated"
                    echo "${dataname_short}.${ab}delim${NODE_DIR}/${dataname}.gzdelimNA" | sed 's/delim/\t/g' >> ${nodenoise_metadata}
		done
            done
	done
    done
	...
	do
		...
		do
                    dataname=${dataname_basic}.${ab}
                    chromo="simulated"
                    echo "${dataname_short}.${ab}delim${B_DIR}/${dataname}.gzdelimNA" | sed 's/delim/\t/g' >> ${boundarynoise_metadata}
		done
            done
	done
    done
	...
	do
		...
		do
		    ...
		    chromo="simulated"
		    echo "${dataname_short1}delim${DD_DIR}/${dataname_basic1}.gzdelimNA" | sed 's/delim/\t/g' >> ${dd_metadata}
		done
	    done
	done
    done
