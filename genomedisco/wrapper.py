        ...
        for line in open(metadata_pairs,'r').readlines():
            ...
            if os.path.isfile(f1) and os.path.getsize(f1)>20:
                if os.path.isfile(f2) and os.path.getsize(f2)>20:
                    ...
                    script_comparison.write("$mypython -W ignore "+os.path.abspath("../genomedisco/genomedisco/__main__.py")+" --m1 "+f1+" --m2 "+f2+" --m1name "+samplename1+" --m2name "+samplename2+" --node_file "+nodefile+" --outdir "+outpath+" --outpref "+chromo+" --m_subsample NA --approximation 10000000 --norm "+norm+" --method RandomWalks "+" --tmin "+str(tmin)+" --tmax "+str(tmax)+concise_analysis_text+'\n')
