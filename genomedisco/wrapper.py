...
def split_by_chromosome(datatype,metadata_samples,outdir,baits,running_mode,nodes):
    ...
    for chromo_line in gzip.open(outdir+'/data/metadata/chromosomes.gz','r').readlines():
        ...
        print 'GenomeDISCO | '+strftime("%c")+' | Processing nodes '+chromo
        ...
        for line in open(metadata_samples,'r').readlines():
            ...
            print 'GenomeDISCO | '+strftime("%c")+' | Processing '+samplename+' '+chromo
...
def run_script(script_name,running_mode):
    if running_mode=='NA':
        ...
        subp.check_output(['bash','-c',script_name])
...
def compute_reproducibility(datatype,metadata_pairs,outdir,norm,tmin,tmax,running_mode,concise_analysis):
    ...
    for chromo_line in gzip.open(outdir+'/data/metadata/chromosomes.gz','r').readlines():
        ...
        for line in open(metadata_pairs,'r').readlines():
            ...
            print 'GenomeDISCO | '+strftime("%c")+' | Processing '+samplename1+'.vs.'+samplename2+' '+chromo
            ...
            if os.path.isfile(f1) and os.path.getsize(f1)>20:
                if os.path.isfile(f2) and os.path.getsize(f2)>20:
                    ...
                    script_comparison.write("$mypython -W ignore "+os.path.abspath("../genomedisco/__main__.py")+" --m1 "+f1+" --m2 "+f2+" --m1name "+samplename1+" --m2name "+samplename2+" --node_file "+nodefile+" --outdir "+outpath+" --outpref "+chromo+" --m_subsample NA --approximation 10000000 --norm "+norm+" --method RandomWalks "+" --tmin "+str(tmin)+" --tmax "+str(tmax)+concise_analysis_text+'\n')
...
def main():
    ...
    global bashrc_file
    bashrc_file=os.path.abspath("../scripts/bashrc_genomedisco")
